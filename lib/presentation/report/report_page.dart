              ...
              Obx(() {
                ...
                if (pieData.isEmpty) {
                  return Card(
                    ...
                    child: Padding(
                      padding: EdgeInsets.all(32),
                      child: Center(child: Text('No data', style: GoogleFonts.poppins(color: Colors.grey))),
                    ),
                  );
                }

                // ✅ Hitung total & normalisasi persentase
                final total = pieData.fold<int>(0, (sum, e) => sum + e.totalBorrowed);
                final normalized = pieData.map((e) {
                  final percent = total > 0 ? (e.totalBorrowed / total) * 100 : 0.0;
                  return {
                    "name": e.name,
                            ...
                            children: [
                              PieChart(
                                PieChartData(
                                  ...
                                  sections: normalized.asMap().entries.map((entry) {
                                    final idx = entry.key;
                                    final data = entry.value;
                                    return PieChartSectionData(
                                      color: figmaColors[idx % figmaColors.length],
