...
class HomeController extends GetxController {
  ...
  var selectedYear = RxnInt(); // bisa null di awal
  ...
  // 🔹 Ambil data loan report berdasarkan tahun
  Future<void> fetchLoanReportByYear(int year, {String? token}) async {
    final loginController = Get.find<LoginController>();
    final authToken = token ?? loginController.token.value;

    // simpan tahun yang dipilih
    selectedYear.value = year;

    // panggil fetchLoanReport dengan from & to = tahun yang sama
  ...
  Future<void> refreshData() async {
    ...
    final token = loginController.token.value;
    if (token.isEmpty) {
      print('Token kosong, tidak bisa refresh data!');
      return;
    }
